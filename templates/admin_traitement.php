...
if ($_SERVER["REQUEST_METHOD"] === "POST" && $_GET["value"] == "save_activity") {
    ...
    $data = [
        ...
        $_POST["DESCRIPTION_CMPLT"],
        $_POST["TYPE_ACT_EVENT"],
        $_POST["DATE_DEBUT"],
        $_POST["DATE_FIN"],
        $_POST["ID_CLIENT_ORGANISATEUR"],
        $_POST["PRIX"],
        $_POST["CAPACITE"],
        $_POST["DIFFICULTE"],
        $_POST["PUBLIC_CIBLE"],
    ...
    if ($id > 0) {
        $sql = "UPDATE ACTIVITE_EVENEMENT 
                SET NOM=?, DESCRIPTION_COURTE=?, DESCRIPTION_CMPLT=?, TYPE_ACT_EVENT=?, DATE_DEBUT=?, DATE_FIN=?, 
                    ID_CLIENT_ORGANISATEUR=?, PRIX=?, CAPACITE=?, DIFFICULTE=?, PUBLIC_CIBLE=?, TAGS=?, VISIBLE=? 
                WHERE ID_ACT_EV=?";
        $data[] = $id;
    } else {
        $sql = "INSERT INTO ACTIVITE_EVENEMENT 
                (NOM, DESCRIPTION_COURTE, DESCRIPTION_CMPLT, TYPE_ACT_EVENT, DATE_DEBUT, DATE_FIN, 
                 ID_CLIENT_ORGANISATEUR, PRIX, CAPACITE, DIFFICULTE, PUBLIC_CIBLE, TAGS, VISIBLE) 
                VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)";
    }
    $stmt = $db->prepare($sql);
    ...
    echo "<script>alert('Activité enregistrée');location.href='admn.php?tab=activities';</script>";
}
